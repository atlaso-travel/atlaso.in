...
/**
 * Computed from real listings, not hand-typed.
 *
 * These previously read "50+ destinations / 500+ operators / 10K+ trips", none
 * of which were true. On an indexable page that is a straightforward liability:
 * it contradicts the counts on every other page and in /llms.txt, and an answer
 * engine that finds two different operator counts on one site discounts both.
 */
const STATS = [
  {
  ...
  {
    value: String(new Set(packages.map((p) => p.operatorId)).size),
    label: "Operators listed",
  },
  { value: String(packages.length), label: "Packages to compare" },
  {
    value: formatPrice(
      Math.round(
        packages.reduce((sum, p) => sum + p.pricing.savings, 0) / packages.length
      )
    ),
    label: "Average saving vs direct",
  },
];
